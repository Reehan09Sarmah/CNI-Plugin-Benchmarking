...
for SIZE in 1 64 256; do
    ...
    for ((i=1; i<=$RUNS; i++)); do
        # Robust parsing: Grabs the line that starts with numbers (the actual data row)
        RESULT=$(kubectl exec netperf-client -- netperf -H $SERVER_IP -t TCP_RR -l $LATENCY_TIME -- -r ${SIZE},${SIZE} 2>/dev/null | grep -E '^[[:space:]]*[0-9]+' | awk '{print $6}')
        
        if [ -z "$RESULT" ]; then 
            echo "    Error on Run $i."
        else
            echo "${SIZE},$i,$RESULT" >> $CSV_FILE
            echo "    Result: $RESULT Trans/sec"
        fi
        sleep 2
    done
done